...
SRC_FILE = 'bible_people/bible_data_GYGJ+NIV/bible_data_test.kr.shuf.tok' # web에서 입력받은 txt token화 된것.
TRG_FILE = 'bible_people/bible_data_GYGJ+NIV/bible_data_test.en.shuf.tok' # 그냥 아무 영어파일. 따로 x
...
def symbolize_kr(line):
	...
	for i in kr_num:
		if isNumber(kr_words[i]):
			...
			replace22(kr_words, kr_num, kr_words[i], symbol[count_num])
...
def symbolize_kr2en(train_kr=SRC_FILE, train_en=TRG_FILE):
	...
	mapping_out=open('mapping.sym','wb')
	...
	while(True):
		...
		kr_words, en_words = NumtoSym_same(kr_words, kr_num, en_words, en_num,mapping_out)
		kr_words, en_words = check1to20(kr_words, kr_num, en_words)


		kr_words = NumtoSym_rest(kr_words, kr_num,mapping_out) # 100 -> __N0
		en_words = NumtoSym_rest(en_words, en_num,mapping_out)

		'''

		#######################################################################################
		-해야할 것.
		1.한국어를 넣으면 -> __N0가 된다.  ----이건 dataset에서 써야 되는 것. 1번은 완료. (나중에 데이터 전처리 과정에 넣어주기)
		ex) 나는 17살이다.   -> 나는 __N0살이다.
		./symbolize_test 해서 나온 kr_line 값.
...
def NumtoSym_same(src_split, src_num_index, trg_split, trg_num_index,mapping_out):
	global COUNT_NUM
	for i in src_num_index:
		for j in trg_num_index:
			if isNumber(src_split[i]) and src_split[i] == trg_split[j]:
				replace2(src_split, src_num_index, src_split[i], symbol[COUNT_NUM],mapping_out)
				replace2(trg_split, trg_num_index, trg_split[j], symbol[COUNT_NUM],mapping_out)
				if COUNT_NUM < MAX_NUM:
					COUNT_NUM += 1
	...
	for i in src_num_index:
		if isInt(src_split[i]) and 1 <= int(src_split[i]) and int(src_split[i]) <= 20 :
			if checking_list[int(src_split[i])] in trg_split:
                                replace2(trg_split, [n for n in range(len(trg_split))], checking_list[int(src_split[i])], symbol[COUNT_NUM],loop_count,mapping_out)
				replace2(src_split, src_num_index, src_split[i], symbol[COUNT_NUM],mapping_out)
				if COUNT_NUM < MAX_NUM:
					COUNT_NUM += 1
	return src_split, trg_split

def NumtoSym_rest(split, num_index,mapping_out):
	global COUNT_NUM
	for i in num_index:
		if isNumber(split[i]):
			replace2(split, num_index, split[i], symbol[COUNT_NUM],mapping_out)
			if COUNT_NUM < MAX_NUM:
				COUNT_NUM += 1
	return split
...
def replace2(split, index, old, new,mapping_out):
